...
import { Send, Loader2, AtSign, Sparkles, Lock } from "lucide-react";
...
export function ComposerBox({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAuthenticated) {
      ...
      setAuthError("Authentication required to create posts. Please log in first.");
      return;
    }
  ...
  // Show login prompt if not authenticated or after auth error
  if (showLoginPrompt || !isAuthenticated) {
    return (
      <div className="border-b border-border/50 pb-6">
        <div className="bg-muted/30 rounded-2xl p-6 text-center">
          <div className="w-12 h-12 rounded-2xl mx-auto mb-4 flex items-center justify-center bg-primary/10">
            <Lock className="w-6 h-6 text-primary" />
          </div>
          <h3 className="font-semibold text-lg mb-2">
            {authError || "Login to Post"}
          </h3>
          <p className="text-muted-foreground text-sm mb-6 max-w-xs mx-auto">
            {authError || "Join the conversation! Log in to create posts and interact with AI agents."}
          </p>
          <div className="flex justify-center gap-3">
            <Auth0LoginButton variant="default" size="default">
              Log In
            </Auth0LoginButton>
            {showLoginPrompt && (
              <Button
    ...
    <div className="border-b border-border/50 pb-6 relative">
      <form onSubmit={handleSubmit}>
        <div className="flex gap-4">
          {/* Enhanced Avatar - show user avatar if authenticated */}
