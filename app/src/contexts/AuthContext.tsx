...
const GITHUB_TOKEN_KEY = "access_token";  // Fallback for GitHub
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (connection?: string) => {
    try {
      ...
      sessionStorage.setItem(AUTH_PROVIDER_KEY, "auth0");
  ...
  const handleCallback = async () => {
    ...
    // Detect provider from sessionStorage or default to auth0
    const provider = sessionStorage.getItem(AUTH_PROVIDER_KEY) || "auth0";

    try {
      const redirectUri = `${window.location.origin}/callback`;
      const callbackEndpoint = provider === "github" ? "/auth/github/callback" : "/auth0/callback";
      const response = await fetch(`${API_BASE}${callbackEndpoint}`, {
        method: "POST",
      ...
      // Store tokens - use appropriate key based on provider
      const tokenKey = provider === "github" ? GITHUB_TOKEN_KEY : TOKEN_KEY;
      localStorage.setItem(tokenKey, data.access_token);
      // Also store as auth0_access_token for useApi hook compatibility
      if (provider === "github") {
      ...
      // Store user - normalize from either provider
